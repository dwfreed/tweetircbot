...
OBJS = conf.o throttler.o ircmessages.o callbacks.o main.o

LIBS = glib-2.0 gthread-2.0 oauth libcurl json-glib-1.0

CC = gcc
CFLAGS = -ggdb3 -Wall -Wextra -D_GNU_SOURCE -I${CURDIR}/libircclient -DTWITTERBOT_VERSION='"$(shell hg id -n)"' $(shell pkg-config --cflags ${LIBS})
...
depcheck:
	...
	@pkg-config --exists 'glib-2.0 >= 2.28.0' || (echo 'You need at least GLib 2.28.0!' && exit 1)
	@pkg-config --exists 'oauth >= 0.9.4' || (echo 'You need at least liboauth 0.9.4!' && exit 1)
	@pkg-config --exists 'libcurl >= 7.16.2' || (echo 'You need at least libcurl 7.16.2!' && exit 1)
	@pkg-config --exists 'json-glib-1.0 >= 0.14.2' || (echo 'You need at least json-glib 0.14.2!' && exit 1)
